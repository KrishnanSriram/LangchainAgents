...
from investment_tools import multiply_numbers, get_recommended_shares, buy_shares, notify_user

def build_multiplier_agent(llm, react_prompt):
  multiplier_tool = [multiply_numbers]
  agent = create_react_agent(llm=llm, tools=multiplier_tool, prompt=react_prompt)
  agent_executor = AgentExecutor(agent=agent, tools=multiplier_tool, verbose=True)
